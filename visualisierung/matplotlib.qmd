...
#| fig-height: 2.6
_x = plt.figure(figsize=(7, 2.6))
_x = plt.scatter(maschinen["laufzeit"], maschinen["verbrauch"], s=10, alpha=0.6)
_x = plt.xlabel("Laufzeit in Stunden")
_x = plt.ylabel("Verbrauch")
plt.show()
```
...
_x = achse.scatter(maschinen["laufzeit"], maschinen["verbrauch"], s=10, alpha=0.6)
...
_x = achse_a.set_title("A")      # eindeutig
_x = plt.title("B")              # landet auf der zuletzt angelegten
...
for achse, name in zip(achsen, typen):
    ...
    _x = achse.scatter(teil["laufzeit"], teil["verbrauch"], s=10, alpha=0.6,
                      color="0.3")
    _x = achse.set_title(f"{name} (n={len(teil)})", fontsize=10)
    _x = achse.set_xlabel("Laufzeit")
_x = achsen[0].set_ylabel("Verbrauch")
plt.tight_layout()
plt.show()
...
_x = achsen[0, 0].plot(range(len(verlauf)), verlauf.to_numpy(), marker="o", lw=1.6,
                      color="0.3")
_x = achsen[0, 0].set_title("plot: Verlauf", fontsize=10)
_x = achsen[0, 1].scatter(maschinen["laufzeit"], maschinen["verbrauch"], s=8,
                         alpha=0.5, color="0.3")
_x = achsen[0, 1].set_title("scatter: Zusammenhang", fontsize=10)
_x = achsen[1, 0].bar(mittel.index, mittel.to_numpy(), color="0.7")
_x = achsen[1, 0].set_title("bar: Vergleich", fontsize=10)
_x = achsen[1, 1].hist(maschinen["verbrauch"], bins=20, color="0.7",
                      edgecolor="white")
_x = achsen[1, 1].set_title("hist: Verteilung", fontsize=10)
plt.tight_layout()
plt.show()
```
...
_x = achsen[0].scatter(maschinen["laufzeit"], maschinen["verbrauch"], s=8,
                      alpha=0.5, color="0.3")
_x = achsen[0].axhline(grenze, ls="--", color="#b2182b")
_x = achsen[0].set_xlim(0, 23)
_x = achsen[0].set_ylim(0, 80)
_x = achsen[0].set_xticks(range(0, 24, 4))
_x = achsen[0].set_title("Grenzen, Ticks, Hilfslinie", fontsize=10)

_x = achsen[1].scatter(maschinen["laufzeit"], maschinen["verbrauch"], s=8,
                      alpha=0.5, color="0.3")
_x = achsen[1].set_yscale("log")
_x = achsen[1].set_title("dieselben Daten, y logarithmisch", fontsize=10)
for achse in achsen:
    _x = achse.set_xlabel("Laufzeit in Stunden")
    _x = achse.set_ylabel("Verbrauch")
plt.tight_layout()
...
_x = achse.scatter(maschinen.loc[~ist_presse, "laufzeit"],
                  maschinen.loc[~ist_presse, "verbrauch"],
                  s=12, alpha=0.7, color="#c7c7c7")
_x = achse.scatter(maschinen.loc[ist_presse, "laufzeit"],
                  maschinen.loc[ist_presse, "verbrauch"],
                  s=12, alpha=0.7, color="#b2182b")
_x = achse.annotate("Pressen: rund 11 Einheiten mehr", xy=(4, 68),
                   color="#b2182b", fontsize=9)
_x = achse.set_title("Pressen verbrauchen bei gleicher Laufzeit deutlich mehr")
_x = achse.set_xlabel("Laufzeit in Stunden")
_x = achse.set_ylabel("Verbrauch in Kilowattstunden")
_x = fig.text(0.01, -0.04, "Quelle: Betriebsdatenerfassung, Stand 2026",
             fontsize=7, color="0.4")
for rand in ("top", "right"):
    _x = achse.spines[rand].set_visible(False)
plt.tight_layout()
plt.show()
```
