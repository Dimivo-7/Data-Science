...
_x = achsen[0].bar(standorte, umsatz, color=verlauf)
_x = achsen[0].set_title("Verlaufspalette")
_x = achsen[1].bar(standorte, umsatz, color=OKABE[:5])
_x = achsen[1].set_title("qualitative Palette")
for achse in achsen:
...
for achse, (titel, pal) in zip(achsen, [("Okabe-Ito (8 Farben)", OKABE),
    ...
    for k, farbe in enumerate(pal):
        _x = achse.bar(k, 1, width=1, color=farbe)
    _x = achse.set_title(titel, fontsize=10)
    _x = achse.axis("off")
plt.tight_layout()
plt.show()
```
...
for achse, (titel, pal) in zip(achsen.ravel(),
    ...
    for k, farbe in enumerate(pal):
        _x = achse.bar(k, 1, width=1, color=farbe)
    _x = achse.set_title(titel, fontsize=9)
    _x = achse.axis("off")
plt.tight_layout()
plt.show()
```
...
for i, (nm, h) in enumerate(farben.items()):
    k = kontrast(h, "#FFFFFF")
    _x = achse.text(0.02, 0.92 - i * 0.13, f"{nm}, Kontrast {k:.2f} : 1",
                   color=h, fontsize=11, transform=achse.transAxes)
_x = achse.set_title("Textfarben auf weissem Grund")
_x = achse.axis("off")
plt.tight_layout()
plt.show()
```
