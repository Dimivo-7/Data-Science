...
for achse, (titel, pal, dicke) in zip(achsen,
                                      [("alles bunt", bunt, dicke_bunt),
                                       ("ein Akzent", mit_akzent, dicke_akzent)]):
    for i in range(8):
        _x = achse.plot(zeit, reihen[i], lw=dicke[i], color=pal[i])
...
> visuellen Gewichts der übrigen, mit einem Akzent das 10.66-Fache. Rund
> 60 Prozent dieses Gewinns entstehen dadurch, dass die anderen Serien
...
_x = achse.plot(monat, test, lw=2, color=AKZENT, label="Test")
_x = achse.plot(monat, kontrolle, lw=2, color="0.45", label="Kontrolle")
_x = achse.axvline(11.5, ls="--", color="black")
_x = achse.legend(frameon=False, loc="upper left")
_x = achse.set_xlabel("Monat")
...
_x = achsen[0].plot(monat[:12], test[:12], lw=2, color="0.35")
_x = achsen[0].set_xlim(0, 23)
_x = achsen[0].set_ylim(min(test.min(), kontrolle.min()),
                       max(test.max(), kontrolle.max()))
_x = achsen[0].set_title("1 Ausgangslage")
_x = achsen[1].plot(monat, test, lw=2, color=AKZENT)
_x = achsen[1].plot(monat, kontrolle, lw=2, color=NEUTRAL)
_x = achsen[1].axvline(11.5, ls="--", color="black")
_x = achsen[1].set_title("2 Spannung")
_x = achsen[2].bar(["Trend", "Kampagne"], [zuwachs - dd, dd],
                  color=[NEUTRAL, AKZENT])
_x = achsen[2].set_title("3 Auflösung")
_x = achsen[2].set_ylabel("Anteil am Zuwachs")
...
for i in range(8):
    _x = achsen[0].plot(zeit, reihen[i], lw=1.4, color=bunt[i], label=f"S{i + 1}")
_x = achsen[0].legend(fontsize=6, ncol=2, frameon=False, loc="upper left")
_x = achsen[0].set_title("mit Legende")
for i in range(8):
    _x = achsen[1].plot(zeit, reihen[i], lw=dicke_akzent[i], color=mit_akzent[i])
_x = achsen[1].set_xlim(0, 27)
_x = achsen[1].text(24, reihen[ziel, -1], f"S{ziel + 1}", color=AKZENT,
                   fontweight="bold")
_x = achsen[1].text(24, reihen[7, -1], "S8", color="0.55", fontsize=9)
_x = achsen[1].set_title("direkt beschriftet")
for achse in achsen:
    _x = achse.set_xlabel("Monat")
    _x = achse.set_ylabel("Wert")
plt.tight_layout()
...
_x = achse.plot(x, reihe, lw=2, color="black")
_x = achse.plot(zukunft, k * zukunft + d, lw=2, color="0.45", label="Trendmodell")
mitte = reihe[-1] + drift * np.arange(1, 25)
rand = 1.96 * sd_aenderung * np.sqrt(np.arange(1, 25))
_x = achse.plot(zukunft, mitte, lw=2, color=AKZENT, label="Irrfahrt mit Drift")
_x = achse.plot(zukunft, mitte + rand, ls="--", color=AKZENT)
_x = achse.plot(zukunft, mitte - rand, ls="--", color=AKZENT)
_x = achse.set_xlim(0, n + 24)
_x = achse.set_ylim(30, 115)
_x = achse.legend(frameon=False, loc="upper left", fontsize=8)
_x = achse.set_xlabel("Monat")
_x = achse.set_ylabel("Wert")
plt.tight_layout()
plt.show()
...
- [x] Das visuelle Gewicht der Zielserie steigt vom 1.45- auf das 10.66-Fache
      der übrigen ~ Richtig, und rund 60 Prozent davon kommen vom Zurücknehmen der
