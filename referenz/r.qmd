...
beiden Sprachreferenzen identisch benannt und gleich sortiert, und auch die
**Zeilen stehen an derselben Stelle**: Wer in der einen
...
| Kopie anlegen | `b <- a` | R kopiert beim Ändern von selbst, siehe [Python-Grundlagen](../programmierung/python-grundlagen.qmd) |
...
| Eigene Klasse | `structure(list(...), class = "k")`, `mittel.k <- function(x, ...)` | S3; mit Verweissemantik `setRefClass()`, siehe [Objektorientierung](../programmierung/python-objektorientierung.qmd) |
...
| Modellauswahl mit AIC | `AIC(modell)`, `BIC(modell)`, `step(modell)` | kleiner ist besser, siehe [Modellauswahl](../statistik/regression/modellauswahl.qmd) |
| Best-Subset-Auswahl | `leaps::regsubsets(y ~ ., data = df)` | ein bestes Modell je Grösse, dann Kriterium wählen |
| Kreuzvalidierung | `boot::cv.glm(df, glm_modell, K = 10)$delta` | `delta[1]` roh, `delta[2]` biaskorrigiert, siehe [Kreuzvalidierung](../statistik/regression/kreuzvalidierung.qmd) |
| Ridge-Regression | `glmnet::cv.glmnet(x, y, alpha = 0)` | `alpha = 0` ist Ridge, `alpha = 1` ist Lasso |
| Lasso | `glmnet::cv.glmnet(x, y, alpha = 1)` | setzt Koeffizienten auf null, siehe [Regularisierung](../statistik/regression/regularisierung.qmd) |
| Elastic Net | `glmnet::cv.glmnet(x, y, alpha = 0.5)` | `alpha` zwischen 0 und 1 mischt Ridge und Lasso |
| Lambda wählen | `fit$lambda.min`, `fit$lambda.1se` | `lambda.1se` liefert das sparsamere Modell |
| Koeffizienten bei gewähltem Lambda | `coef(fit, s = "lambda.1se")` | dünnbesetzte Matrix, viele Einträge sind null |
...
| Beispieldaten sprachübergreifend | deterministisch erzeugen statt `rnorm()` | in dieser Sammlung über einen Lehmer-Generator, siehe [Reproduzierbarkeit](../programmierung/reproduzierbarkeit.qmd) |
