...
| Modellauswahl mit AIC | `modell.aic`, `modell.bic` | R-Werte liegen um 2 bzw. log(n) höher, siehe [Modellauswahl](../statistik/regression/modellauswahl.qmd) |
| Kreuzvalidierung | `cross_val_score(modell, X, y, cv=10, scoring="neg_mean_squared_error")` | **negatives** MSE, Vorzeichen umdrehen, siehe [Kreuzvalidierung](../statistik/regression/kreuzvalidierung.qmd) |
| LOOCV | `cross_val_score(modell, X, y, cv=LeaveOneOut())` | `from sklearn.model_selection import LeaveOneOut` |
| Ridge-Regression | `RidgeCV(alphas=...).fit(X, y)` | `alpha` in sklearn = `lambda` in R, siehe [Regularisierung](../statistik/regression/regularisierung.qmd) |
| Lasso | `LassoCV(cv=10).fit(X, y)` | setzt Koeffizienten auf null |
| Elastic Net | `ElasticNetCV(l1_ratio=0.5, cv=10).fit(X, y)` | `l1_ratio` in sklearn = `alpha` in glmnet |
| Koeffizienten nach Regularisierung | `modell.coef_`, `modell.alpha_` | `alpha_` ist das gewählte Lambda |
...
| Beispieldaten sprachübergreifend | deterministisch erzeugen statt `rng.normal()` | in dieser Sammlung über einen Lehmer-Generator, siehe [Reproduzierbarkeit](../programmierung/reproduzierbarkeit.qmd) |
